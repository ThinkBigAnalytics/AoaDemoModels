...
score.batch <- function(data_conf, model_conf, model_version, ...) {
    ...
    data <- table %>% select(c("NumTimesPrg", "PlGlcConc", "BloodP", "SkinThick", "TwoHourSerIns", "BMI", "DiPedFunc", "Age")) %>% mutate_if(bit64::is.integer64, as.integer) %>% as.data.frame()
    ...
    patientIds <- table %>% select("PatientId") %>% mutate_if(bit64::is.integer64, as.integer) %>% as.data.frame()
